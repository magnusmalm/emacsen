...
If `USE-INITIAL-MAJOR-MODE' is non-nil, set `major-mode' to `initial-major-mode'."
  (interactive "P")
  (if use-initial-major-mode
      (crux-create-scratch-buffer)
    (scratch major-mode)))
(bind-key "<f7>" 'mmm/create-scratch-buffer)
...
(use-package scratch
  :config
  (defun set-org-mode-scratch-mode ()
    (when scratch-buffer
      (save-excursion
        (goto-char (point-min))
        (insert "#+TITLE: Scratch\n\n"))))
  (add-hook 'org-mode-hook 'set-org-mode-scratch-mode))
